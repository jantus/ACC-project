...
def createWorker(workerNumber):
  ...
  ######## Create Worker #######
  ##############################
  worker = nc.servers.create(name="MavaServerLab3-worker"+str(workerNumber), image=image, flavor=flavor, userdata=worker_userdata_file, key_name=keypair.name, network=network_a)
