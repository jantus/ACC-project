...
app = Celery('airfoilWorker',backend='amqp',broker='amqp://asd:asd@130.238.29.145:5672/asd')
...
def createResFolder(resPath):
    if os.path.isdir(currentDir+'/'+resPath):
        ...
        if os.path.isdir(currentDir+'/'+resPath+'/'+msh):
            for file in os.listdir(resPath+'/msh'):
                os.remove(resPath+'/results/'+file)
            os.rmdir(resPath + '/msh')
        if os.path.isdir(currentDir+'/'+resPath+'/'+geo):
...
def createMesh(resPath,angle,n_angles,n_nodes,n_level):
...
@app.task
def runairfoil(resPath,nSamples,nu,v,t,angle,n_angles,n_nodes,n_level):
    os.chdir(currentDir)
    createResFolder(resPath)
    outfile = createMesh(resPath,angle,n_angles,n_nodes,n_level)
